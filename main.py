...
async def on_message(message: discord.Message):
    ...
    if message.content.startswith("p!aniversariantes"):
        ...
        # Monta lista com avatar + nome + data (apenas membros do servidor)
        lista = []
        for uid, info in aniversarios.items():
            try:
            ...
            if membro:
                lista.append({
                    ...
                    "data": info.get("data", "??/??"),
                    "avatar": getattr(membro.display_avatar, "url", None)
                })
        ...
        class Paginador(discord.ui.View):
            ...
            def _embed(self):
                ...
                for item in paginas[self.ind]:
                    e.add_field(name=f"{item['nome']}", value=f"🎂 {item['data']}", inline=False)

                # thumbnail da página: pega o avatar do primeiro item da página, se existir
                thumb = paginas[self.ind][0].get('avatar') if paginas[self.ind] else None
                if thumb:
            ...
            def _update_buttons(self):
                # habilita/desabilita botões com segurança
                self.first.disabled = self.prev.disabled = (self.ind == 0)
                self.next.disabled = self.last.disabled = (self.ind == len(paginas) - 1)
